...
可选 ASR 烟雾测试：安装 `.[asr]` 后运行 `python tests/asr_smoke.py`。该测试使用 FFmpeg 的 flite 滤镜合成英文语音，首次下载 tiny.en 模型，实际完成识别、时间戳导入及带字幕的成片渲染。GitHub Actions 在独立 Ubuntu 作业中执行；它验证适配链路，不代表中文识别准确率评测或 GPU 兼容性认证。
